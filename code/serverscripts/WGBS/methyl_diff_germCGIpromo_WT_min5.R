...
difffiles <- Sys.glob("../results/methylKit_5min/WT*_CpG.txt")
difffiles
#make as a list
samp_list <- list()
...
METHoverBED <- function(bedfile, outname){
	...
	write.table(dm.regions, paste0("../results/methylKit_5min/WGBS_restab_regionCounts_", outname,"_WT_min5_ESCvsEpiLC.csv"), sep = ',', row.names = FALSE, quote = FALSE)
	...
	write.table(myDiff_regions, paste0("../results/methylKit_5min/WGBS_getmethyldiff_", outname,"_p10_min5_q1_WT_ESCvsEpiLC.csv"), sep = ',', row.names = FALSE, quote = FALSE)
...
bedgraph(myDiff10p, col.name = "WT ESC vs EpiLC min5 q1 unpooled", file.name = "../results/methylKit_5min/bedgraph_diff_WT_ESCvsEpiLC_p10_min5_q1_notpooled.bed")  
