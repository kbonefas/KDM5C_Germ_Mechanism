...
#precent methylation - percMethylation
...
difffiles <- Sys.glob("../results/methylKit/*_CpG.txt")
difffiles
#make as a list
samp_list <- list()
...
METHoverBED <- function(bedfile, outname){
	...
	#do the methylation percentage calculation
	percmeth <- percMethylation(united_regions)
	print("percmeth united_regions")
	print(head(percmeth))

	#combine with united regions
	perc_df <- data.frame(getData(united_regions)[,1:3], percmeth)
	print("percmeth dataframe")
	print(head(perc_df))
	write.table(perc_df, paste0("../results/methylKit/WGBS_percmeth_", outname,"_min3.csv"), sep = ',', row.names = FALSE, quote = FALSE)
