...
samples <- c("ESC","EpiLC", "PNC") #make sure order matches snakefile
...
ggsave(snakemake@output[[length(samples) + 2]], grid.arrange(grobs = plots, ncol = length(samples)), width = 10, height = 3.5)


### Which germline genes are bound by KDM5C
germ <- read.csv(snakemake@input[[length(samples) + length(DEGsample)]], sep = ",")
# print("germ genes")
# # print(head(germ))
...
write.table(genestatus, snakemake@output[[length(samples) + 3]], sep = ",", col.names = FALSE, row.names = FALSE, quote = FALSE)
#save list of genes for HOMER
write.table(subset(genestatus, KDM5C_binding == "Bound")[,"SYMBOL"], snakemake@output[[length(samples) + 4]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
write.table(subset(genestatus, KDM5C_binding == "Unbound")[,"SYMBOL"], snakemake@output[[length(samples) + 5]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
...
#save table
write.table(germ, snakemake@output[[length(samples) + 6]], sep = ",", row.names = FALSE, quote = FALSE)
#save list of genes for HOMER
write.table(subset(germ, KDM5C_binding == "Bound")[,"SYMBOL"], snakemake@output[[length(samples) + 7]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
write.table(subset(germ, KDM5C_binding == "Unbound")[,"SYMBOL"], snakemake@output[[length(samples) + 8]], sep = "\t", col.names = FALSE, row.names = FALSE, quote = FALSE)
...
library("ggplot2")
ggsave(snakemake@output[[length(samples) + 9]], plot = p, width = 4, height = 3)
