...
####################################################################################################################
################### 2025.08.06 - ICC plots for Stra8 #############################
library("readxl")
STRA8_100nMRA <- read_excel(snakemake@input[[2]])
print(head(STRA8_100nMRA))
...
#make a new column with genotype and treatment to compare
STRA8_100nMRA$genotreat <- paste(STRA8_100nMRA$Genotype, STRA8_100nMRA$Treatment)
#make a new column with the percent of DAPI cells that are DAZL+
STRA8_100nMRA$Percent <- (STRA8_100nMRA$STRA8/STRA8_100nMRA$DAPI*100)
...
#list of sample names
STRA8_100nMRA$ID <- paste(STRA8_100nMRA$Sample, STRA8_100nMRA$genotreat)
allsamps <- unique(STRA8_100nMRA$ID)
dflist <- list()

for (k in 1:length(allsamps)){
    #get the dataframe for one sample
    df <- subset(STRA8_100nMRA, STRA8_100nMRA$ID == allsamps[k])
    #print(df)
    newdf <- df[1,3:ncol(df)]
    print(newdf)
...
library(dplyr)
STRA8_avg <- bind_rows(dflist)

print(head(STRA8_avg))
