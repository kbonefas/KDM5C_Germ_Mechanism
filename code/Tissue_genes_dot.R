...
p <- ggplot(tissuecount, aes(x=tissue, y = sample, color = pvalue, size = DEGnumb)) + labs(x = NULL, y = NULL) +
  geom_point() + scale_color_gradient(low="red", high="blue") + theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 

ggsave(snakemake@output[[1]], plot = p, width = 8, height = 4)
